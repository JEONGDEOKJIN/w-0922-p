(() => {
  ...
  const moduleActionHandler = {
    ...
    MOVE_TO_PARENT_POST: ({ target }) => {
      const parentArticleNo = target.closest('[shopby-parent-article-no]')?.getAttribute('shopby-parent-article-no');
      searchParams.set('articleNo', parentArticleNo);

      location.href = `/pages/board/post-detail.html?${searchParams.toString()}`;
    },
